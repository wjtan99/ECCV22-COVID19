...
parser.add_argument('--gpu_id',type = str,default='0,1,2,3', help='foo help')
...
def main():
    ...
    if args.evaluate:
        ...
        model,HEAD = build_model_validate()
        ...
        model, HEAD,startEpoch, optimizer, best_prec1 = build_model_continue()
    ...
    if args.evaluate:
        prec1,prec3,lossClassification = validate(val_loader, model,HEAD, criterion,criterion2,modality)
        return
...
#        adjust_learning_rate(optimizer, epoch)
        prec1_train, prec3_train = train(train_loader, model, HEAD, criterion,criterion2, optimizer, epoch,modality)
...
def build_model():
    ...
    if args.dataset=='ucf101':
        ...
        print('model path is: %s' %(model_path))
        num_classes = 2 
        model = models.__dict__[args.arch](modelPath=model_path, num_classes=2, length=args.num_seg)
    ...
    model = model.cuda()
    HEAD = ArcFace(in_features = 512, out_features = num_classes, device_id =[0]) # gpu_id)
...
def build_model_validate():
    ...
    if args.dataset=='ucf101':
        ...
        model=models.__dict__[args.arch](modelPath='', num_classes=2,length=args.num_seg)  
        num_classes = 2 
    if torch.cuda.device_count() > 1:
        model=torch.nn.DataParallel(model) 
    ...
    HEAD = ArcFace(in_features = 512, out_features = num_classes, device_id = gpu_id)
...
def build_model_continue():
    ...
    return model, HEAD, startEpoch, optimizer, best_prec




def train(train_loader, model, HEAD, criterion, criterion2, optimizer, epoch,modality):
    ...
    for i, (inputs, targets, scans, slices) in enumerate(train_loader):
        ...
        #print("Start traning i = {}, scan = {}, target = {}".format(i,scans, targets)) 
        ...
        #print("output type = {}, shape = {}".format(type(output),output.shape))  
...
def validate(val_loader, model, HEAD, criterion,criterion2,modality):
    batch_time = AverageMeter()
    lossesClassification = AverageMeter()
    top1 = AverageMeter()
    top3 = AverageMeter()
    ...
    if print_output:
        if args.test: 
            fp = open("BERT-test-results-split{}.txt".format(args.split),"w") 
        else:
            fp = open("BERT-validate-results-split{}.txt".format(args.split),"w") 
    ...
    with torch.no_grad():
        for i, (inputs, targets, scans, slices) in enumerate(val_loader):
            ...
            # compute output
            output, input_vectors, sequenceOut, _, embedding = model(inputs)
