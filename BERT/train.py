...
def build_model():
    ...
    if args.dataset=='ucf101':
        ...
        print('model path is: %s' %(model_path))
        model = models.__dict__[args.arch](modelPath=model_path, num_classes=4, length=args.num_seg)
...
def build_model_validate():
    ...
    if args.dataset=='ucf101':
        ...
        model=models.__dict__[args.arch](modelPath='', num_classes=4,length=args.num_seg)  
...
def validate(val_loader, model, criterion,criterion2,modality):
    ...
    if print_output:
        if args.test: 
            fp = open("BERT-test-results-split{}-{}.txt".format(args.split,args.modelfile),"w") 
        else:
            fp = open("BERT-validate-results-split{}-{}.txt".format(args.split,args.modelfile),"w") 
    gt_all = [] 
    pred_all = [] 
