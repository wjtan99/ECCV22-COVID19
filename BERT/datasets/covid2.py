...
def ReadFeature(root, path, length):
 
    feature_file = "{}features/{}.npy".format(root,path) 
    #print(feature_file) 
    ...
    if len(feat.shape)==0:
        print("Bad feature file = ", feature_file) 


    new_feat = np.zeros((length, feat.shape[1])).astype(np.float32)
    r = np.linspace(0, len(feat), length+1, dtype=np.int)
    #r = random.randint(0, feat.shape[0]-1)
...
def ReadSegmentRGBMaskResample(root, path, offsets, new_height, new_width, new_length, is_color, name_pattern, duration,start,stop,is_train,bbox):
    ...
    if is_train and False:
        ...
        interval = int(len(slices_all) // new_length)
        if interval>=1:
            offset = random.randint(0, interval-1)-interval+1 
        else:
            offset = 0 
        #offset = 0 #previous default 
        slices = SymmetricalResampler.resample(slices_all, new_length, offset)
...
class covid2(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.modality == "rgb" or self.modality == "CNN":
            ...
            clip_input = ReadFeature(self.root, path, self.new_length)
            slices = [] 
        else:
        ...
        if self.modality == "feature":
            clip_input = torch.tensor(clip_input) 
