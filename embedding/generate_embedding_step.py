...
def main():
    ...
    if True:
        ...
        for line in tqdm(val_list):
            ...
            output_file = args.dataset_root + 'features_0524_step16/{}.npy'.format(line.split(' ')[0])
            ...
            while not finished:
                ...
                if seg_end>num_imgs: 
                    seg_end = num_imgs 
                    seg_start = max(seg_end - length,0) 
                    finished = True 
                #print(finished,seg_start,seg_end)                
                ...
                if len(slices)<length and len(slices)>0:
                    slices = SymmetricalResampler.resample(slices, length)
            ...
            if len(slices_to_process) == 0:
                print("line = {}, slices = 0".format(line)) 
                input("Check") 
